...
class KMeans(BaseEstimator):
    ...
    source: <https://github.com/rushter/MLAlgorithms/blob/master/mla/kmeans.py>
    ...
    #         palette = sns.color_palette("hls", self.K + 1)[i]
    #         ax.scatter(*point, c=palette[0])
    #
    ...
    def plot(self, ax=None, holdon=False):
        ...
        p = figure(title="K-means Example")

        colors = ["#222222", "#ee6666", "#aa22aa", "#880011"]
        for i, index in enumerate(self.clusters):
            points = data[index]
            self.mscatter(p, zip(*points)[0], zip(*points)[1], "circle", color=colors[i])

        for point in self.centroids:
            self.mscatter(p, point[0], point[1], "square", color="#223388")

        # p.set(x_range=Range1d(0, 10), y_range=Range1d(0, 10))
        if not holdon:
            show(p)

    def mscatter(p, x, y, typestr, color="#ee6666"):
