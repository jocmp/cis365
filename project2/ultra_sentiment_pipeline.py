...
df = pd.read_csv(Constants.PARSED_DATA)
...
train_lower_limit = split + 1

X_train = df.loc[train_lower_limit:training_size, 'review'].values
y_train = df.loc[train_lower_limit:training_size, 'sentiment'].values

X_test = df.loc[test_lower_limit:test_upper_limit, 'review'].values
y_test = df.loc[test_lower_limit:test_upper_limit, 'sentiment'].values
